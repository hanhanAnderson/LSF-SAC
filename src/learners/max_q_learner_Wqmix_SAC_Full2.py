...
class MAXQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.log_alpha =th.tensor(-0.7, dtype=th.float32,requires_grad=True)
        self.log_alpha.requires_grad=True
        # self.alpha = th.ones(1, dtype=th.float32)
        self.alpha = th.tensor(0.2, dtype=th.float32)
        self.alpha_optimizer = th.optim.Adam([self.log_alpha], lr=3e-4)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        alpha_loss = (th.sum(mac_out*(-1.0* (actions_logprobs+ target_entropy)),dim=-1)[:,:-1]*mask).sum()/mask.sum()
        ...
        loss = policy_loss + self.args.qmix_loss * qmix_loss + self.args.central_loss * central_loss + comm_loss
        ...
        alpha_loss = Variable((th.sum(mac_out.detach() * (-1.0 * (actions_logprobs + target_entropy).detach()), dim=-1)[:,
                      :-1] * mask).sum() / mask.sum(),requires_grad = True)
