...
class MAXQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.alpha = th.tensor(0.2, dtype=th.float32)
        ...
        self.target_mac = copy.deepcopy(self.critic_mac)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # mac_out -> action_probs
        z = mac_out == 0.0
        z = z.float() * 1e-8
        # RuntimeError: The size of tensor a (128) must match the size of tensor b (90) at non-singleton dimension 1
        ...
        for t in range(batch.max_seq_length):
            target_agent_outs, (mu_c, sigma_c), logits_c, m_sample_c = self.target_mac.forward(batch, t=t)
        ...
        # target_max_agent_qvals = self.target_mixer(target_max_agent_qvals, batch["state"][:, 1:])
        target_max_agent_qvals = self.target_mixer(target_max_agent_qvals, batch["state"])
        # We use the calculation function of sarsa lambda to approximate q star lambda
        targets = build_td_lambda_targets(rewards, terminated, mask, target_max_agent_qvals,
        ...
        # Q_i_mean_negi_mean = Q_i_mean_negi_mean.repeat(repeats=(1, 1, self.args.n_agents))[:,:-1] # (1,60,3)
        # mask = mask.expand_as(Q_i_mean_negi_mean)
        # policy_loss = ((Q_i_mean_negi_mean* mask)**2).sum() / mask.sum()
        ...
        #             # print("actionn: ", actionn)
            ...
            # th.cuda.empty_cache()
        # if t_env > 500000 and (not th.equal(cur_max_actions_cGlobalMax, cur_max_actions)):
            # print("cur_max_actions_cGlobalMax is Greater THAN cur_max_actions!!!" )
            # pass
        #label_target_max_out[avail_actions[:, :-1] == 0] = -9999999
        #label_target_actions = label_target_max_out.max(dim=3, keepdim=True)[1]
        label_target_actions =  cur_max_actions[:,:-1]
        ...
        # loss = policy_loss + self.args.qmix_loss * qmix_loss + self.args.central_loss * central_loss + 0.01 * comm_loss


        #
        """
        
        iinput = th.flatten(mac_out[:,:-1],0,1)
        ttartget = th.flatten(actions,0,1).squeeze()     
        lsum = 0.
        for e in range(iinput.shape[0]):
            lsum += ce_loss(iinput[e],ttartget[e])
        lsum/mask.sum()
        """
        loss =  masked_td_error.sum() / mask.sum() + comm_loss * 0.1
        ...
        #Alpha_loss
        target_entropy = -1. * self.args.n_actions
        # alpha_loss = (th.sum(mac_out.detach()*(-1.0* (actions_logprobs+ target_entropy).detach()),dim=-1)[:,:-1]*mask).sum()/mask.sum()
        # alpha_loss = Variable((th.sum(mac_out.detach() * (-self.log_alpha * (actions_logprobs + target_entropy).detach()), dim=-1)[:,:-1] * mask).sum() / mask.sum(),requires_grad = True)
    ...
    def _update_targets(self):
        self.target_mac.load_state(self.critic_mac)
