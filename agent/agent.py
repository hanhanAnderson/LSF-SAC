...
class Agents:
    ...
    def choose_action(self, obs, last_action, agent_num, avail_actions, epsilon, maven_z=None, evaluate=False):
        ...
        phidden_state = self.policy.peval_hidden[:, agent_num, :]
        ...
        if self.args.alg == 'maven':
            ...
            ################## USING POLICY ACTION################################
            agent_outs, self.policy.peval_hidden[:, agent_num, :] = self.policy.eval_pagent(inputs, phidden_state)
        ...
        if self.args.alg == 'coma' or self.args.alg == 'central_v' or self.args.alg == 'reinforce':
            ...
            if evaluate:
                action = torch.argmax(agent_outs)  # 概率分布
                # action = q_value.max(dim=1, keepdim=False)[1]
            if np.random.uniform() < epsilon:
                action = np.random.choice(avail_actions_ind)  # action是一个整数
            else:
                action = torch.argmax(agent_outs)
                # action = torch.argmax(q_value)
                # q_value = torch.softmax(q_value / 1.0, dim=1)  # 概率分布
                # action = np.random.choice(np.arange(q_value.shape[-1]), 1,
                #                           p=q_value.squeeze(0).detach().cpu().numpy())  # action是一个整数 按概率分布采样
